...
public class MinMax extends RustlerInterface{
		...
		for(int i=0; i<7 ;  i++){
			for(int j=0; j<7; j++){
				if(board[i][j].getPiece() instanceof Rider &&  board[i][j].getPiece().getColor()==1){
					playingPiece = board[i][j].getPiece();
					destinationlist = playingPiece.move(board, 3, 3);
					bestMove =  destinationlist.get(0);
					break;
				}
			}
		}
	...
	public String makeMove(Board b) {
		...
		for(int i = 0; i<7; i++){
			for(int j=0; j<7; j++){
				if(b.hasPiece(i,j)){
						Move move = new Move(i,j,0,0,playingPiece);
						moves.add(move);
	...
	public String doMove(Board b, Move moveToMake){
		 ...
		 */
		if(pieceToMove instanceof Horse){
			if(pieceToMove.getColor() == 0){//case that it is white horse
				if(b.hasPiece(7, 7)){
					if(pieceToMove instanceof Rider){
	...
	public int evaluatePosition(Board b, int alpha, int beta, int depth, boolean color){ 
		...
		if(color == false){ //minimizing player--sequence of events that occurs
			...
			for(int i = 0; i<8; i++){
				for(int j=0; j<8; j++){
					if(b.hasPiece(i,j)){
						if(b.getSquare(i,j).getPiece().getColor() == 1){
							Piece piece = b.getSquare(i,j).getPiece();
							for(int k =0; k<8; k++){
								for(int l=0; l<8; l++){
			...
			for(int i = 0; i<8; i++){
				for(int j=0; j<8; j++){
					if(b.hasPiece(i,j)){
						if(b.getSquare(i,j).getPiece().getColor() == 0){
							Piece piece = b.getSquare(i,j).getPiece();
							for(int k =0; k<8; k++){
								for(int l=0; l<8; l++){
									Point p = new Point(k*62,l*62);
		...
		for(int i = 0; i<7; i++){
			for(int j=0; j<7; j++){
				if(b.hasPiece(i, j)){
					...
					}else if(b.getSquare(i, j).getPiece().getColor() == 1){ //case that piece is white
						if(b.getSquare(i,j).getPiece() instanceof Rider){
							blackscore += 9;
						}else if(b.getSquare(i,j).getPiece() instanceof Horse){
							blackscore += 10000000;
